...
import (
	...
	_ "github.com/lrascao/etcdkeeper/session/providers/memory"
)
...
func main() {
	...
	middleware := func(fns ...func(s *session.Manager, w http.ResponseWriter, r *http.Request)) func(w http.ResponseWriter, r *http.Request) {
	...
	fassets, err := fs.Sub(assets, "assets")
	if err != nil {
		log.Fatal(err)
	}

	http.Handle("/", http.FileServer(http.FS(fassets))) // view static directory
...
func nothing(_ *session.Manager, _ http.ResponseWriter, _ *http.Request) {
	// Nothing
}

// v3 api
func connect(sessmgr *session.Manager, w http.ResponseWriter, r *http.Request) {
	mu.Lock()
	defer mu.Unlock()
...
func put(sessmgr *session.Manager, w http.ResponseWriter, r *http.Request) {
	cli := getClient(sessmgr, w, r)
	defer cli.Close()
	key := r.FormValue("key")
...
func get(sessmgr *session.Manager, w http.ResponseWriter, r *http.Request) {
	data := make(map[string]interface{})
	key := r.FormValue("key")
...
func getPath(sessmgr *session.Manager, w http.ResponseWriter, r *http.Request) {
...
func del(sessmgr *session.Manager, w http.ResponseWriter, r *http.Request) {
	cli := getClient(sessmgr, w, r)
	defer cli.Close()
	key := r.FormValue("key")
	dir := r.FormValue("dir")
...
func getSeparator(sessmgr *session.Manager, w http.ResponseWriter, _ *http.Request) {
	io.WriteString(w, separator)
}

func getClient(sessmgr *session.Manager, w http.ResponseWriter, r *http.Request) *clientv3.Client {
	sess := sessmgr.SessionStart(w, r)
	v := sess.Get("uinfo")
	if v != nil {
		uinfo := v.(*userInfo)
